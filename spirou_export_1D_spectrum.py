...
rvdata = ascii.read(options.rvfile,data_start=2)
...
for i in range(len(inputedata)) :
    ...
    output = inputedata[i].split(".")[-2] + "_s.fits"
    ...
    if options.verbose :
        print("Exporting spectrum:",inputedata[i]," output=", output, "{0}/{1}".format(i,len(inputedata)-1))
    ...
    # set initial source RV as the one from input ccf file
    spectrum["source_rv"] = rvs[i]
    spectrum['RV'] = rvs[i]
    BJD_mid = spectrum['header1']["BJD"] + (spectrum['header0']['MJDEND'] - spectrum['header0']['MJD-OBS']) / 2.
    BERV = float(spectrum['header1']['BERV'])
    snr32 = spectrum['header1']['SNR32']
    airmass = spectrum['header0']['AIRMASS']
    ...
    header.set('ORIGIN', "spirou_export_1D_spectrum.py")
    header.set('UTCSAVED', time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()))

    header.set('BJDMID', BJD_mid, "Barycentric Julian Date at middle of exposure")
    header.set('BERV', BERV, "Barycentric Earth Radial Velocity correction (km/s)")
    header.set('SNR32', snr32, "Signal-to-Noise ratio for order 32")
    header.set('AIRMASS', airmass, "Airmass")
    header.set('RV', spectrum['RV'], "Radial velocity of source (km/s)")
    header.set('RVFILE', options.rvfile, "Radial velocity data file")

    header.set('NORDERS', NORDERS, "Number of spectral orders")
    ...
    hdu_wl = fits.ImageHDU(data=out_wl, name="WAVE")
    outhdulist.append(hdu_wl)
    
    hdu_flux = fits.ImageHDU(data=out_flux, name="FLUX")
    outhdulist.append(hdu_flux)
        
    hdu_fluxerr = fits.ImageHDU(data=out_fluxerr, name="FLUXERR")
    outhdulist.append(hdu_fluxerr)

    hdu_order = fits.ImageHDU(data=out_order, name="ORDER")
